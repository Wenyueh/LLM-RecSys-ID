...
from item_rep_method import (
    ...
    no_tokenization,
    item_resolution,
    no_tokenization_for_user,
)
from utils import load_meta, find_metadata, load_reviews
import time
import json
import numpy as np
import pickle as pkl
from base_conversion import binary, ternary, quaternary, identity
...
def load_data(args, tokenizer):
    ...
    if args.item_representation == "item_resolution":
        randomize_fct = random_number(12102)
        base_conversion_fct = identity
        if args.number_base in [2, 3, 4]:
            if args.number_base == 2:
                base_conversion_fct = binary
            elif args.number_base == 3:
                base_conversion_fct = ternary
            else:
                assert args.number_base == 4
                base_conversion_fct = quaternary
        remap_fct = lambda x: item_resolution(
            resolution=args.resolution, overlap=args.overlap
        )(base_conversion_fct(randomize_fct(x)))
    if args.item_representation == "no_tokenization":
        remap_fct = no_tokenization()
    if args.user_representation == "no_tokenization":
        user_remap_fct = no_tokenization_for_user()

    for one_user in data:
        splittion_point = one_user.index(" ")
        user = one_user[:splittion_point]
        items = one_user[splittion_point + 1 :].split(" ")
        if remap_fct is not None:
            remapped_items = [remap_fct(item) for item in items]
        else:
            remapped_items = items
        if user_remap_fct is not None:
...
class sequential_item_dataset(Dataset):
    ...
    def __len__(self):
        ...
        # number = len(self.users) * self.args.sequential_num * self.num_template
    ...
    def __getitem__(self, index):
        ...
        end_candidates = [
            _
            for _ in range(
                max(1, len(sequence) - self.args.sequential_num - 1), len(sequence) - 1,
            )
        ]
        end_index = random.randint(0, len(end_candidates) - 1)
        end_pos = end_candidates[end_index]
        start_candidates = [_ for _ in range(0, min(2, end_pos))]
...
def load_eval_dataloaders(
    ...
    if method == "sequential_item":
        ...
        eval_direct_yesno_dataset = evaluation_direct_yesno_dataset(
            args, users, all_items, eval_sequence, task_subgroup_3, mode
        )
        # create sampler
        if args.distributed:
        ...
        eval_direct_candidate_dataset = evaluation_direct_candidate_dataset(
            args, users, all_items, eval_sequence, task_subgroup_4, mode
        )
        # create sampler
        if args.distributed:
...
class category_dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        template_index = index % self.num_templates
        template = self.templates[template_index]
        inputs = template["source"].format("item_" + remapped_item)
...
class review_item_dataset(Dataset):
    def __init__(
        self, args, reviews, all_items, task_subgroup_10, remap_all_items,
    ):
        super().__init__()
        self.args = args
    ...
    def __getitem__(self, index):
        ...
        text = review["text"]
        user_id = review["user"]
        if self.args.user_representation == "no_tokenization":
            user_id = "<user_id_{}>".format(user_id)
        item_id = review["item"]
...
class review_user_dataset(Dataset):
    def __init__(
        self, args, reviews, all_items, task_subgroup_11, remap_all_items,
    ):
        super().__init__()
        self.args = args
        self.reviews = reviews
...
def meta_loader(args, tokenizer, all_items, remapped_all_items):
    ...
    pretrain_review_item_dataloader = DataLoader(
        ...
        batch_size=args.pretrain_review_item_batch,
        collate_fn=collator,
        shuffle=(sampler is None),
        sampler=sampler,
    )
    ...
    pretrain_review_user_dataloader = DataLoader(
        ...
        batch_size=args.pretrain_review_user_batch,
        collate_fn=collator,
        shuffle=(sampler is None),
        sampler=sampler,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pretrain_title_batch", type=int, default=10)
    parser.add_argument("--pretrain_description_batch", type=int, default=10)
    parser.add_argument("--pretrain_category_batch", type=int, default=10)
    parser.add_argument("--pretrain_review_item_batch", type=int, default=10)
    parser.add_argument("--pretrain_review_user_batch", type=int, default=10)
    ...
    train_review_user_dataset = review_user_dataset(
        args, reviews, all_items, task_subgroup_11, remapped_all_items,
    )
    train_review_item_dataset = review_item_dataset(
        args, reviews, all_items, task_subgroup_10, remapped_all_items,
    )
    # for i in tqdm(range(len(train_review_user_dataset))):
    #    a = train_review_user_dataset[i]
    print(tokenizer.tokenize(train_review_user_dataset[1]))
    print(tokenizer.tokenize(train_review_user_dataset[2]))
    print(tokenizer.tokenize(train_review_user_dataset[3]))
    print(tokenizer.tokenize(train_review_user_dataset[4]))

    # for i in tqdm(range(len(train_review_item_dataset))):
    #    a = train_review_item_dataset[i]
    ...
    (
        ...
        train_direct_straightforward_dataloader,
        task_data_lengths,
    ) = load_train_dataloaders(args, tokenizer)

    for batch in train_direct_yesno_dataloader:
        input_ids = batch[0]

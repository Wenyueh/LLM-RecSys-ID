...
def pretrain(args, logger, rank, model, pretrain_loaders, optimizer, scheduler):
    if rank == 0:
...
def trainer(
    ...
    if args.use_meta_data:
        model = pretrain(
            args, logger, rank, model, pretrain_loaders, optimizer, scheduler
        )
...
def parse_argument():
    ...
    parser.add_argument(
        ...
        help="random_number, random_one_token, no_tokenization, None",
    )
    parser.add_argument(
